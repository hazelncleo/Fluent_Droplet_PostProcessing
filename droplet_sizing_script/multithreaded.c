...
void calculate_droplets(){
        ...
        real droplet_values[7] = {0.}; // 0 = vol, 1-3 = centroid, 4-6 = velocity
        real calc_values[3] = {0.}; // 0 = vof, , 1 = mass, 2 = temp value
        droplet_id[0] = myid + 1;
        Message("\nNode %d initialized\n", myid);
            ...
            if (FLUID_THREAD_P(cell_thread)){
                ...
                begin_c_loop(cell, cell_thread){
                    ...
                    if (droplet_id[1] == 0 && calc_values[0] > 0.5) { 
                        ...
                        PRF_CSEND_REAL(node_host, droplet_values, 7, droplet_id[0]);
            ...
            for (i = 0; i < compute_node_count; i++){
                if (nodes_completed[i] == 0){
                    ...
                    if (message == -1){
                        ...
                        PRF_CRECV_REAL(i, droplet_values, 7, droplet_id);
                        fprintf(
                            fptr, 
                            "%d,%e,%e,%e,%e,%e,%e,%e\n",
                            droplet_id,
